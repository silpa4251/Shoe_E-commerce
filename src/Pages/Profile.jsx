...
const Profile = () => {
    ...
    const [prodata,setProdata] = useState(initialProData());

    useEffect(() => {
      if(user) {
        setProdata((prevData) => ({...prevData, username: user}))
      }
    },[user]);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        localStorage.setItem("profile",JSON.stringify(prodata));
        console.log("Profile Updated:", prodata);
    };
  return (
    <div className="max-w-2xl mx-auto bg-white shadow-lg rounded-lg p-8 mt-10">
    <h1 className="text-4xl font-bold text-center mb-6 text-gray-700">Welcome, {prodata.username}</h1>
